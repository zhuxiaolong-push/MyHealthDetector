...
#define SMBUS_SCK_H()	    PBout(10)=1 //置高电平
#define SMBUS_SCK_L()	    PBout(10)=0  //置低电平
#define SMBUS_SDA_H()	    PBout(11)=1
#define SMBUS_SDA_L()	    PBout(11)=0

#define SMBUS_SDA_PIN()	   PBin(11) //读取引脚电平


/**************************************GXY*******************************************/
//// 定义引脚位偏移
//#define SMBUS_SDA_PIN     11
#define SMBUS_SDA_MODE    (11 * 2)

// PB11 输入模式
#define SMBUS_SDA_IN()  do { \
    GPIOB->MODER &= ~(3 << SMBUS_SDA_MODE); \
    GPIOB->MODER |= (0 << SMBUS_SDA_MODE); \
    delay_us(1); \
} while(0)

// PB11 输出模式
#define SMBUS_SDA_OUT() do { \
    GPIOB->MODER &= ~(3 << SMBUS_SDA_MODE); \
    GPIOB->MODER |= (1 << SMBUS_SDA_MODE); \
    delay_us(1); \
} while(0)
